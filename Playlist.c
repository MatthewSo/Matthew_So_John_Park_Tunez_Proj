...
void add_song(table t, char * nname, char * nartist) {
  ...
  t[i] = insert_inOrder(nname, nartist, t[i]);
}
...
void print_artist(table t, char * artist) {
  int i = getIndex(artist[0]);
  song_node * temp = t[i];
  while (temp != 0) {
    if (strcmp(artist,temp->artist) == 0) {
      print_single(temp);
    }
    temp = temp->next;
  }
...
void shuffle(table t) {
  int i = 0;
  for (;i < 26; i++) {
    song_node * temp = random_node(t[i]);
    if (temp != 0) {
      print_single(temp);
    }
  }
...
void delete_song( table t, char * name, char * artist){
  ...
  for(;i<26;i++){
    if( t[i] != 0 ){
      song_node * temp = remove_song( name, artist, t[i]);
      t[i] = temp;
    }
  }
}
